...
float move_speed = 0.018f, sensitivity = 0.006f;
...
int __cdecl hook_CameraFunction(float* a1, int a2) {
    if (!enabled && a1) {
        ...
        a1[8] = sy * cp; a1[9] = -sp; a1[10] = cy * cp;
        a1[12] = cam_x; a1[13] = cam_y; a1[14] = cam_z;
    }
...
DWORD WINAPI InputThread(LPVOID param) {
    ...
    while (true) {
        ...
        if (enabled) {
            ...
            if (GetAsyncKeyState(VK_DOWN) & 0x8000)  move_speed = max(0.001f, move_speed - 0.001f);
...
DWORD WINAPI MainThread(LPVOID param) {
    ...
    HANDLE h = CreateThread(NULL, 0, InputThread, NULL, 0, NULL);
    if (h) CloseHandle(h);
    return 0;
}
